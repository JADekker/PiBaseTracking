...
def url : String := "https://github.com/JADekker/PiBaseTracking"
...
/-- Encodes the status that a definition can have-/
inductive StatusDefinition
  | missing
  | notStated
  | stated
  deriving DecidableEq, Hashable

instance : ToString StatusDefinition where
  toString
  | .missing => "missing"
  | .notStated => "not stated"
  | .stated => "stated"

/-- Encodes the status that a result can have-/
inductive StatusResult
  | missing
  | notStated
  | stated
  | proved
...
instance : ToString StatusResult where
  toString
  | .missing => "missing"
  | .notStated => "not stated"
  | .stated => "stated"
...
/-- Derives if a given `d` has been proven-/
...
/-- Get the module name for a given name -/
def getModuleNameFor? (env : Environment) (nm : Name) : Option Name :=
  env.getModuleIdxFor? nm >>= fun i => env.header.moduleNames[i.toNat]?

/-- Get the filepath, memory position and status of declaration, if possible-/
def info_for (n : Name) : Lean.CoreM (Option (System.FilePath × Name) × Option DeclarationRange × Status) := do
  let e ← getEnv -- get current environment
  let some d := e.find? n | return (none, none, .missing) -- if we can find name n, return it. Otherwise report it as missing.
  let s ← status_of d -- get the status of the definition
  let fn := getModuleNameFor? e n -- try to get the module name
  let p := DeclarationRanges.range <$> (← Lean.findDeclarationRanges? n) -- ? find the memory position declaration
  let fs ← match fn with -- Get the filepath where the declaration lives
  | none => pure none
  | some fn => (do
...
def getTitle (n : Name) : CoreM String := do
  let some doc := Lean.getModuleDoc? (←getEnv) n | throwError "no docstring" -- tries to get the docstring
  if h : 0 < doc.size then -- ? if the doc.size is greater than 0, extract the name of the declaration from the docstring
    let s := (doc[0]'h).doc
    let i := s.find (· = '#')
